...
class MiniPhotoshopApp:
    def __init__(self, root):
        ...
        self.crop_area = None
        self.has_cropped = False   # track whether a crop has been applied
    ...
    def _update_canvas(self):
        ...
        orig_resized = cv2.resize(orig, (nw1, nh1))
        pil1 = Image.fromarray(orig_resized)
        ...
        self.canvas.create_text(
            x1_off + nw1//2, y1_off + nh1 + 12,
            text=configs.original_label_text,
            font=configs.label_font,
            fill=configs.label_color
        )
        self._tk_orig = tk1  # keep reference

        # Draw edited (right) *only* if a crop has been applied
        if self.has_cropped:
            ...
            pil2 = Image.fromarray(curr_resized)
            ...
            self.canvas.create_text(
                x2_off + nw2//2, y2_off + nh2 + 12,
                text=configs.edited_label_text,
                font=configs.label_font,
                fill=configs.label_color
            )
            self._tk_edit = tk2  # keep reference
    ...
    def resize_image(self, val):
        ...
        self.current_image = cv2.resize(self.current_image, (int(w*scale), int(h*scale)))
        self._update_canvas()
